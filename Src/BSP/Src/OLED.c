...
void OLED_WriteCommand(uint8_t Command)
{
    ...
    HAL_I2C_Master_Transmit(0x78, data, 2, 100);
}

uint8_t OLED_WriteData(uint8_t *Data, uint16_t Count)
{
    if (Count > 1024)
    {
        return 1;
    ...
    {
        static uint8_t data[1025];
        data[0] = 0x40;
        memcpy(&data[1], Data, Count);

#ifdef Use_DMA
        if (OLED_Transfer_Done_Flag)
        {
            HAL_I2C_Master_Transmit_DMA(&hi2c1, 0x78, data, Count + 1);
            OLED_Transfer_Done_Flag = 0;
        }

#else
        HAL_I2C_Master_Transmit(0x78, data, Count + 1, 100);
#endif
    }
    return 0;
}
...
static void OLED_Delay(uint16_t ms)
{
    Delay_Us(ms);
}
...
void OLED_Init(void)
{
    ...
    OLED_WriteCommand(0xB0); // 0x00~0xFF	  //1010 0000
    // A[3:0] : Define the divide ratio D
    // A[7:4] : Set the Oscillator Frequency

    OLED_WriteCommand(0xA8); // 设置多路复用率
    OLED_WriteCommand(0x3F); // 0x0E~0x3F //64mux

    OLED_WriteCommand(0xD3); // 设置显示偏移
    OLED_WriteCommand(0x00); // 0x00~0x7F

    // 使用内部RAM自增地址模式 可能会导致上次传输未完成  OLED 内部列/页地址未清零
    // 所以需要设置页和列地址
    OLED_WriteCommand(0x21); // 设置列起始和结束地址
    OLED_WriteCommand(0);    // 起始列
    OLED_WriteCommand(127);  // 结束列

    OLED_WriteCommand(0x22); // 设置页起始和结束地址
    OLED_WriteCommand(0);    // 起始页
    OLED_WriteCommand(7);    // 结束页

    OLED_WriteCommand(0x20); // 水平地址模式
...
void OLED_Flip(u8 dir)
{
    if (dir)
    {
        OLED_WriteCommand(0xA0); // 锟斤拷锟斤拷锟斤拷锟揭凤拷锟斤拷0xA1锟斤拷锟斤拷锟斤拷0xA0锟斤拷锟揭凤拷锟斤拷
        OLED_WriteCommand(0xC0); // 锟斤拷锟斤拷锟斤拷锟铰凤拷锟斤拷0xC8锟斤拷锟斤拷锟斤拷0xC0锟斤拷锟铰凤拷锟斤拷
    }
    else
    {
        OLED_WriteCommand(0xA1); // 锟斤拷锟斤拷锟斤拷锟揭凤拷锟斤拷0xA1锟斤拷锟斤拷锟斤拷0xA0锟斤拷锟揭凤拷锟斤拷
        OLED_WriteCommand(0xC8); // 锟斤拷锟斤拷锟斤拷锟铰凤拷锟斤拷0xC8锟斤拷锟斤拷锟斤拷0xC0锟斤拷锟铰凤拷锟斤拷
    }
}
...
void OLED_Update(void)
{
    ...
    if (OLED_Transfer_Done_Flag)
    {
        static uint8_t data[1025];
        data[0] = 0x40;
        memcpy(&data[1], OLED_DisplayBuf[0], 1024);
        HAL_I2C_Master_Transmit_DMA(&hi2c1, 0x78, data, 1025);
        OLED_Transfer_Done_Flag = 0;
        tempfps++;
    }
...
{
    OLED_WriteCommand(0x26 | direction); // 0x26=向右,0x27=向左
    OLED_WriteCommand(0x00);             // 虚拟字节(必须为0)
    OLED_WriteCommand(start_page);       // 起始页(0-7)
    OLED_WriteCommand(speed);            // 滚动速度(0-7,0=最慢)
    OLED_WriteCommand(end_page);         // 结束页(0-7)
    OLED_WriteCommand(0x00);             // 虚拟字节(必须为0)
    OLED_WriteCommand(0xFF);             // 虚拟字节(必须为0xFF)
}

// 设置垂直+水平滚动(对角线滚动)
void OLED_SetDiagonalScroll(uint8_t direction, uint8_t start_page, uint8_t end_page, uint8_t speed, uint8_t vertical_offset)
{
    OLED_WriteCommand(0x29 | direction); // 0x29=向右上,0x2A=向左上
    OLED_WriteCommand(0x00);             // 虚拟字节
    OLED_WriteCommand(start_page);       // 起始页
    OLED_WriteCommand(speed);            // 滚动速度
    OLED_WriteCommand(end_page);         // 结束页
    OLED_WriteCommand(vertical_offset);  // 垂直偏移量(0-63)
}

// 启用/禁用滚动
void OLED_ActivateScroll(uint8_t enable)
{
    OLED_WriteCommand(enable ? 0x2F : 0x2E);
}
 ...
 * 说    明:调用此函数后,要想真正地呈现在屏幕上,还需调用更新函数
 */
void OLED_ShowFloatNum_Nosigned(int16_t X, int16_t Y, double Number, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
